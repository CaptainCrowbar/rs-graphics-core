...
void test_rs_graphics_core_colour_space_ciexyy() {
    ...
    for (auto& sample: samples) {
        ...
        TEST(is_colour_in_gamut<CIExyY>(colour));
...
void test_rs_graphics_core_colour_space_wide_gamut() {
    ...
    for (auto& sample: samples) {
        ...
        TEST_VECTORS(cie, sample.CIEXYZ, 1e-6);
    }
...
void test_rs_graphics_core_colour_space_hsl() {
    ...
    for (auto& sample: samples) {
        ...
        TRY(rgb = HSL().to_base(polar));
        TRY(cie = LinearRGB().to_base(rgb));
        TEST_VECTORS(cie, sample.CIEXYZ, 1e-10);
    }
