...
namespace RS::Graphics::Core {
    ...
    private:
        ...
        template <typename T, int N>
        std::string Box<T, N>::str(const RS::Format::FormatSpec& spec) const {
    ...
    private:
        ...
        typename Sphere<T, N>::real_type Sphere<T, N>::volume() const noexcept {
            ...
            if constexpr (N == 1) {
                ...
                return pi<R> * radius_ * radius_;
            } else if constexpr (N == 3) {
                static const R c = pi<R> * R(4) / R(3);
                return c * radius_ * radius_ * radius_;
            } else {
                static const R log_pi = std::log(pi<R>);
        ...
        typename Sphere<T, N>::real_type Sphere<T, N>::surface() const noexcept {
            ...
            if constexpr (N == 1) {
                ...
                return R(2) * pi<R> * radius_;
            } else if constexpr (N == 3) {
                return R(4) * pi<R> * radius_ * radius_;
            } else {
                static const R log_pi = std::log(pi<R>);
                static const R a = R(N) / R(2);
        ...
        template <typename T, int N>
        std::string Sphere<T, N>::str(const RS::Format::FormatSpec& spec) const {
            return 'S' + centre_.str(spec) + '+' + RS::Format::format_object(radius_, spec);
        }
