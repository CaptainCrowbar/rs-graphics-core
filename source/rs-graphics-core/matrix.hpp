...
namespace RS::Graphics::Core {
    ...
    RS_DEFINE_ENUM_CLASS(MatrixLayout, int, 0, row, column)
    ...
    public:
        ...
        template <typename... Args, typename U = T>
            constexpr Matrix(T x, std::enable_if_t<TL::SfinaeTrue<U, sizeof...(Args) + 2 == cells>::value, T> y, Args... args):
    ...
    template <typename T, int N, MatrixLayout L>
    std::string Matrix<T, N, L>::str(const RS::Format::FormatSpec& spec) const {
        std::string s;
        for (int r = 0; r < N; ++r) {
            s += r ? ',' : '[';
            for (int c = 0; c < N; ++c)
                s += (c ? ',' : '[') + RS::Format::format_object((*this)(r, c), spec);
            s += ']';
        }
        s += ']';
        return s;
...
namespace std {
    ...
    template <typename T, int N, RS::Graphics::Core::MatrixLayout L>
    struct hash<RS::Graphics::Core::Matrix<T, N, L>> {
        size_t operator()(const RS::Graphics::Core::Matrix<T, N, L>& m) const noexcept { return m.hash(); }
    };
